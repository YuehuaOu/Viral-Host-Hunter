...
def parse():
    parser = argparse.ArgumentParser(
        description=(
            "Train the Viral-Host-Hunter model for gut-associated phage host prediction.\n\n"
    ...
    parser.add_argument(
        ...
        help='Protein type used for model training: "tail"or "lysin".'
    )
    parser.add_argument(
        '--output_dir', type=str,
        help="Directory to save trained model. (default: ./model/gut_prophages/{type}/{level})"
    )
    parser.add_argument(
        ...
        help=(
            ...
            "(default: ./embeddings/gut_prophages/{type}/{level})"
        )
    )
    parser.add_argument(
...
def main():
    ...
    if args.embedding_dir is not None:
        ...
        embedding_file = os.path.join('.', 'embedding', 'gut_prophages', type, level)

    if args.output_dir is not None:
        output_dir = args.output_dir
    else:
        output_dir = os.path.join('.', 'model', 'gut_prophages', type, level)
