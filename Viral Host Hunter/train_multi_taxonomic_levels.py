...
type = args.type
level = args.level
embedding_file = f"./embedding/multi_taxonomic_levels/{type}/{level}/"
...
utils.save_pickle(standard_scaler, f"./model/multi_taxonomic_levels/{type}/{level}/standard_scaler.pkl")
...
for epoch in range(1, config.epochs + 1):
    ...
    if val_acc > best_acc:
        ...
        best_acc = val_acc
        torch.save(DHH.state_dict(), f"./model/multi_taxonomic_levels/{type}/{level}/model.pth")
    else:
...
# train autoencoder
DHH = DnaPathNetworks().to(config.device)
DHH.load_state_dict(torch.load(f"./model/multi_taxonomic_levels/{type}/{level}/model.pth"))
...
for epoch in range(1, 150 + 1):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(autoencoder.state_dict(), f"./model/multi_taxonomic_levels/{type}/{level}/autoencoder.pth")
...
autoencoder = AutoEncoder(input_dim, hidden_dims).to(config.device)
autoencoder.load_state_dict(torch.load(f"./model/multi_taxonomic_levels/{type}/{level}/autoencoder.pth"))
...
for min_samples_leaf in [1, 2, 3, 4]:
    for min_samples_split in [2, 3, 4]:
        for n_estimators in [50, 100, 150, 200]:
            ...
            if val_acc > best_acc:
                best_acc = val_acc
                utils.save_pickle(clf, f"./model/multi_taxonomic_levels/{type}/{level}/rf.pth")
